...
PanelWindow {
    ...
    //    semantics. ──
    readonly property bool autoApproveActive: popupState.phase === "confirming"
        && popupState.autoApprove && !root.dismissed
    property real autoApproveElapsedMs: 0
    readonly property real autoApproveProgress: autoApproveActive
        ? Math.min(1.0, autoApproveElapsedMs / (popupState.autoApproveTimeoutSecs * 1000))
        : 0
    ...
    Timer {
        ...
        onTriggered: {
            ...
            if (root.autoApproveElapsedMs >= popupState.autoApproveTimeoutSecs * 1000) {
                stop();
