...
//
// Unchanged from the standalone quickshell/AnimatedBorder.qml this
// plugin ports -- it has no window/layer-shell/host dependency of its
// own, just OmarchyTheme (this directory's own copy) for colors.
//
...
// PopupCard.qml) so the card's own opaque fill covers the hole,
// leaving only the ring-and-glow frame visible around it.
//
// Colors: the gradient stops and every glow color come from
// OmarchyTheme (see OmarchyTheme.qml), matching PopupCard.qml's own
...
//     isn't listening for anything (see Service.qml), so a
